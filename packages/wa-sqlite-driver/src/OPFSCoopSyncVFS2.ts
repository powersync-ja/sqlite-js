...
const finalizationRegistry = new FinalizationRegistry<() => void>((releaser) =>
  releaser()
);
...
interface FileSystemSyncAccessHandle extends FileSystemHandle {
  truncate(n: number): void;
  read(buffer: Uint8Array<ArrayBufferLike>, options: { at: number });
  flush(): void;
  write(buffer: Uint8Array<ArrayBufferLike>, options: { at: number });
  getSize(): number;
  close(): void;
}
...
class PersistentFile {
  ...
  handleLockReleaser: null | (() => void) = null;
...
export class OPFSCoopSyncVFS2 extends FacadeVFS {
  ...
  jRead(fileId: number, pData: Uint8Array, iOffset: number): number {
    try {
      ...
      const accessHandle =
        file.accessHandle || file.persistentFile.accessHandle;
