...
const getTrials = (searchTerms) => {
  ...
  return require('./trials').searchable.filter(
...
const sortTrials = (a, b) => {
  ...
  if (typeof aRo !== 'boolean')
    throw new TypeError(`Missing readonly export in benchmark type ${a.type}`);
  if (typeof bRo !== 'boolean')
    throw new TypeError(`Missing readonly export in benchmark type ${b.type}`);
  return bRo - aRo;
};

const displayTrialName = (trial) => {
  if (trial.description)
    return console.log(clc.magenta(`--- ${trial.description} ---`));
  const name = `${trial.type} ${trial.table} (${trial.columns.join(', ')})`;
  const pragma = trial.customPragma.length
    ? ` | ${trial.customPragma.join('; ')}`
    : '';
  console.log(clc.magenta(name) + clc.yellow(pragma));
};

const createContext = (trial, driver) => {
  const tableInfo = Object.assign({}, tables.get(trial.table), {
    data: undefined
  });
  return JSON.stringify(
    Object.assign({}, trial, tableInfo, { driver, tables: [...tables.keys()] })
  );
};
...
const nameLength = [...drivers.keys()].reduce(
...
for (const trial of trials) {
  ...
  for (const driver of drivers.keys()) {
    ...
    try {
      console.log('node', ['./benchmark.js', ctx]);
      const result = execFileSync('node', ['./benchmark.js', ctx], {
        stdio: 'pipe',
        encoding: 'utf8'
      });
      ...
      console.log(
        erase() + clc.red(`${driverName} ERROR (probably out of memory)`)
      );
