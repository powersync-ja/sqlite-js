...
exports['node-sqlite3'] = async (db, { table, columns, driver, pragma }) => {
  ...
  const row = Object.assign(
    {},
    ...Object.entries(await db.get(`SELECT * FROM ${table} LIMIT 1`))
  ...
  /*
	 ...
	 */
  return () =>
    open('../temp/benchmark.db', pragma).then(async (db) => {
      try {
        await db.run('BEGIN');
        try {
          ...
          try {
            ...
            /* this is necessary because there's no db.inTransaction property */
          }
          throw err;
        }
      } finally {
...
exports['sjp'] = async (db, { table, columns }) => {
  const sql = `INSERT INTO ${table} (${columns.join(', ')}) VALUES (${columns.map((x) => '@' + x).join(', ')})`;
  const row = (await db.select(`SELECT * FROM ${table} LIMIT 1`))[0];
